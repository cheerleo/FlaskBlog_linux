...
def set_reset_request():
    # （1）如果用户已登录，进入url:"/reset_password"将直接重定向至set_home的页面
    #【注意】 需要去掉已登录就不让修改密码的情况，因为我们在account页面也要增加重置密码功能
    #if current_user.is_authenticated:
    #    return redirect(url_for("main.set_home"))
...
def set_reset_token(token):
    # （1）如果用户已登录，进入url:"/reset_password/<token>"将直接重定向至set_home的页面
    #【注意】 需要去掉已登录就不让修改密码的情况，因为我们在account页面也要增加重置密码功能  
    # if current_user.is_authenticated:
    #     return redirect(url_for("main.set_home"))
    ...
    if reset_form.validate_on_submit():
       ...
       # 如果原本用户是已登录状态，则跳转到account页面
        if current_user.is_authenticated:
            return redirect(url_for("users.set_account"))
       # 如果原本用户是未登录状态，则跳转到login页面
        else:
            return redirect(url_for("users.set_login"))
